...
from mne.utils import sum_squared, warn
...
def wrap_skcuda_fft(x, n=None):
    n = None
    print("n: '%s'" % n)
    print("x.shape: '%s'" % str(x.shape))
    orig_x = x
    x_gpu = gpuarray.to_gpu(x)
    xf_gpu = gpuarray.zeros((x.shape[0], x.shape[1]), np.complex64)
    plan = skcuda.fft.Plan(x.shape, np.float32, np.complex64)
    reg_res = scipy.fftpack.fft(orig_x)
    print("reg_res shape: '%s'" % str(reg_res.shape))


    skcuda.fft.fft(x_gpu, xf_gpu, plan)
    cuda_res = xf_gpu.get()
    print("cuda_res shape: '%s'" % str(cuda_res.shape))

    print(reg_res)
    print("*****************")
    print(cuda_res)
    print(np.allclose(reg_res[:, 0:xf_gpu.shape[1]], cuda_res, atol=1e-4))

    print("------------------------------------")
    return reg_res

def wrap_skcuda_ifft(x):

    x_gpu = gpuarray.to_gpu(x)
    xf_gpu = gpuarray.empty([n // 2 + 1 for n in x.shape], np.complex64)
    plan = skcuda.fft.Plan(x.shape, np.complex64, np.float32)
    skcuda.fft.ifft(x_gpu, xf_gpu, plan)

    return xf_gpu.get()

fft = wrap_skcuda_fft
# ifft = wrap_skcuda_fft
ifft = scipy.fftpack.ifft
fftfreq = fftpack.fftfreq
...
def _mt_spectra(x, dpss, sfreq, n_fft=None):
    ...
    for idx, sig in enumerate(x):
        x_mt[idx] = fft(sig[np.newaxis, :] * dpss,
                                n=n_fft)[:, freq_mask]
    return x_mt, freqs
...
def _psd_multitaper(x, sfreq, fmin=0, fmax=np.inf, bandwidth=None,
    ...
    freqs = fftfreq(x.shape[1], 1. / sfreq)
